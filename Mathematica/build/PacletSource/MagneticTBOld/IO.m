...
pythtbIO2dold[f_,wc_,rule_,lattold_]:=Module[{wccold,
nband,toexp,onsiteham,onsitehopping,neighhopping,tmp1,tmp2,aold,bold,
...
(*Print[onsitehopping];*)
neighhopping=Map[Values[(GroupBy[Cases[#,xold_ E^yold_:>{xold,Expand[yold]}],Last])]&,toexp,{2}];
(*Print[neighhopping];*)
neighhopping=Map[{Total[#[[;;,1]]],#[[1,2]]}&,neighhopping,{3}];
neighhopping=Table[
tmp1=neighhopping[[i,j]];
Table[
tmp2=Values[First@SolveAlways[({aold,bold}-wccold[[i]]+wccold[[j]]) . {I kxold,I kyold}==bond[[2]],{kxold,kyold}]];
...
plotslabold[wc_,lattold_,shape_]:=Module[{wccold,latticeold,atomposold,pos,nfind,
...
nfind=30;
wccold=wc[[;;,;;2]];
latticeold=lattold[[;;2,;;2]];
(*atompos=Flatten[Table[{{i,j},#+{i,j}&/@wcc},{i,-nfind,nfind},{j,-nfind,nfind}],1];*)
atomposold=Flatten[Table[{{i,j,#[[1]],#[[2]]}&/@({#,wccold[[#]]}&/@Range[Length[wccold]])},{i,-nfind,nfind},{j,-nfind,nfind}],3];
shapepos=Select[atomposold,shape@@((#[[;;2]]+#[[4]]) . latticeold)&];
plotpos=((#[[;;2]]+#[[4]]) . latticeold)&/@N@shapepos
...
Options[hopold] = {"wcc"->None,"hrExport"->None};
hopold[f_,rule_, OptionsPattern[]]:=Module[{toexp,hopp,hoppm,onsite,select,hr,band,aold,bold,cold,hopset,hopsetM,nums,sstr,result,
hrsim,rehrsim,hk,pij,
integerstr,
realstr,hrstr,
wc,export},
...
xx_ E^yy_:>{Sequence@@First[SolveAlways[yy/I==(-wc[[i]]+wc[[j]]+{aold,bold,cold}) . {kxold,kyold,kzold},{kxold,kyold,kzold}]][[;;,2]],i,j,ComplexExpand@Re@xx,ComplexExpand@Im@xx}],{i,band},{j,band}],2];
(*Print[hopp];*)
hoppm=Flatten[Table[Cases[{Collect[toexp[[i,j]],E^__]},
xx_ E^yy_:>{Sequence@@First[SolveAlways[yy/I==(-wc[[i]]+wc[[j]]+{aold,bold,cold}) . {kxold,kyold,kzold},{kxold,kyold,kzold}]][[;;,2]],i,j,ComplexExpand@Re@xx,ComplexExpand@Im@xx}],{i,band},{j,band}],2];
(*hopp=SortBy[hopp,{#[[{1,2,3,5,4}]]&}]*)
...
(*Print[onsite];*)
onsite=Select[onsite,Norm[#[[-2;;]]]=!=0&&Norm[#[[-2;;]]]=!=0.&];
(*Print[onsite];*)
(*Print[onsite];*)
hopp=SortBy[Join[hopp,hoppm,onsite],{#[[{1,2,3,4,5}]]&}];
...
hrsim={Take[#,3],Take[#,{4,5}],#[[6]]+I #[[7]]}&/@result;
rehrsim={#[[1]]+pij[#[[2]][[1]],#[[2]][[2]]],#[[2]],#[[3]]}&/@hrsim;
hk=Normal[Total[SparseArray[#[[2]]->#[[3]]Exp[I #[[1]] . {kxold,kyold,kzold}],{band,band}]&/@rehrsim]];
(*If[Normalize[hk-f]\[NotEqual]0,Print[hk-f]];*)
(*If[Norm@Simplify[hk-(f/.rule)]!=0,Print["Error"];Abort[],Print["pass"],
Print[(*Norm@*)Norm@Chop@Simplify[hk-(f/.rule)]]];*)
nums=Length[hopset];
...
Options[readHRold] = {"prec" -> 10^-5, "ncell" -> 3, "efermi" -> 0, 
   "wcc" -> None};
readHRold[hrdat_, OptionsPattern[]] := 
 Module[{hr, nband, nBandSqare, nAutom, nL, degenerate, hrSim, smallhr,
   prec = OptionValue["prec"],
   ncell = OptionValue["ncell"],
   efermi = OptionValue["efermi"],
   wccold = OptionValue["wcc"],
   del0,
   delcell,
   hk,
   hkk0
  ...
  del0[xold_] := True;
  del0[0] = False;
  Scan[(delcell[#] = True) &, Tuples[Range[-ncell, ncell], 3]];
  
  hr = Drop[ReadList[hrdat, String], 1];
  nband = ToExpression[hr[[1]]];
  If[wccold === None, wccold = Table[0, {nband}]];
  nBandSqare = nband^2;
  nAutom = ToExpression[hr[[2]]];
  nL = Ceiling[nAutom/15] - 1;
  degenerate = 
   ToExpression /@ Flatten[StringSplit /@ Take[hr, {3, 3 + nL}]];
  hrSim = {Take[#, 3], Take[#, {4, 5}], 
      Chop[#[[6]] + I #[[7]], 
       prec]} & /@ (ToExpression /@ (StringSplit /@ 
        Take[hr, {4 + nL, -1}]));
  hrSim = Partition[hrSim, nBandSqare];
  (*Print[Length[hrSim]];*)
  hrSim = Select[#, del0[#[[3]]] && delcell[#[[1]]] &] & /@ hrSim;
  smallhr = {};
  Do[
   (*Print[hrSim[[b]]];*)
   If[hrSim[[bold]] != {},
    Do[AppendTo[smallhr, MapAt[#/degenerate[[bold]] &, cold, 3]]
     , {cold, hrSim[[bold]]}]
    ]
   , {bold,(*10*)nAutom}];
  (*smallhr;*)
  hk = SparseArray[{}, {nband, nband}];
  Do[
   (*Print[ele];*)
   hk[[ele[[2, 1]], ele[[2, 2]]]] += 
    ele[[3]] Exp[I (ele[[1]] - wccold[[ele[[2, 1]]]] + 
          wccold[[ele[[2, 2]]]]) . {kxold, kyold, kzold}]
   , {ele, smallhr}];
