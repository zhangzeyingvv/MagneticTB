...
bandManipulateold[pathstr_,npoint_,h_]:=Module[
  {h0,params,mparams,m,rule,mpstr,klist},
  h0=TrigToExp[h];
  params=Variables[h0];
  mparams=ToExpression["p"<>ToString@Hash[#,"SHA"]<>ToString[#]<>"old"&/@params];
...
(*  Print[mparams,m];*)
  mpstr="Manipulate[\[IndentingNewLine]ListPlot[Transpose@Table[Eigenvalues[Evaluate["<>ToString[h,StandardForm]<>"/."<>rule<>"~Join~{MagneticTBOld`kxold\[Rule]k\[LeftDoubleBracket]1\[RightDoubleBracket],MagneticTBOld`kyold\[Rule]k\[LeftDoubleBracket]2\[RightDoubleBracket],MagneticTBOld`kzold\[Rule]k\[LeftDoubleBracket]3\[RightDoubleBracket]}]],{k,"<>ToString[klist]<>"}],
...
Options[bandplotold]= {plotRangeold->All};
bandplotold[pathstr_,npoint_,ham_,rule_,OptionsPattern[]]:=Module[{tmp,kps,hkps,hkp,xticks,yticks,data},
...
(*  tmp=ToExpression@Partition[Partition[StringSplit[pathstr],5][[;;,1;;3]],2];*)
  ...
  Transpose@Table[Sort@Eigenvalues[Evaluate[(N@ham)/.rule/.{kxold->kps[[k,1]],kyold->kps[[k,2]],kzold->kps[[k,3]]}]],{k,Length[kps]}]
    ,{i,Length[tmp]}];
...
(*PlotRange->OptionValue[PlotRange],*)
    PlotRange->{{0,(npoint)(Length[hkp]-1)},OptionValue[plotRangeold](*{-.02,.02}*)},
    PlotStyle->Black,
...
banddataold[pathstr_,npoint_,ham_,rule_,save_]:=Module[{tmp,kps,hkps,hkp,xticks,yticks,data},
