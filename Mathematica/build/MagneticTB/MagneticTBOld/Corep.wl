...
getMSGElemFromMSGCorepold[MSG_]:=Module[
{SG,brav,msgelem,lattold},
...
SG=MSG[[1]];
brav=getSGLatt[SG];
lattold=BasicVectors[brav]/.{
SpaceGroupIrep`a->MagneticTBOld`aold,
SpaceGroupIrep`b->MagneticTBOld`bold,SpaceGroupIrep`c->MagneticTBOld`cold,
SpaceGroupIrep`\[Alpha]->MagneticTBOld`alphaold,
SpaceGroupIrep`\[Beta]->MagneticTBOld`betaold,
SpaceGroupIrep`\[Gamma]->MagneticTBOld`gammaold};
(*Print["Magnetic space group (BNS):",MSGSymStd[MSG]," No. ",StringRiffle[MSG,"."]];
Print["Lattice: ",brav];*)
Print["Primitive Lattice Vactor: ",lattold];
...
rk=Association@MapIndexed[First@#2->#1&,Insert[#,Inverse@Transpose[#[[2]]],5]&/@symminfoold];
(*Print[rk];*)
...
getTBBandCorepold[MSG_, ham_, param_, kset_] := Module[
  {rot,tmp, opsold, wc, sym, tr, brav, msgele,mpgele,srot,srottb, eiv, little, trace, cr, coeff,
   U, opII, actk,
   order,sign
   },
  tr = Association[];
  opsold = N@symmetryopsold;
  (*ops[[19]]=-ops[[19]];*)
  ...
  tr["soc"] =If[If[MissingQ@(basisdictold[#]&@basisold[[1,1]]),ListQ[basisold[[1,1]]],
  ListQ[(basisdictold[#]&@basisold[[1,1]])]
  ],1,0];
  tr["nsym"] = Length[sym];
...
(*  msgele = getMSGElem[MSG];
  ...
  srottb= spinMatrix2[Transpose[lattold] . # . Inverse@Transpose@lattold] & /@ rot;
...
(*  Print[Table[sym[[i,1]]->MatrixForm[FullSimplify@srot[[i]]],{i,Length[rot]}]];
  ...
  Do[
   ...
   eiv = Eigensystem[
     ConjugateTranspose[
       U] . (ham /. 
        Join[param, Thread[{kxold, kyold, kzold} -> 2 Pi  kpoint]]) . U];
   ...
   trace =
    Flatten[Table[
      Table[
       Table[
        actk = Inverse[Transpose[symminfoold[[i, 2]]]];
        ...
        opII = 
          Exp[-2 Pi I sym[[i, 3]] . (actk . (kpoint))] Table[
           Exp[2 Pi I actk . 
            ...
            Length[wc]}, {l, Length[wc]}] opsold[[i]];
            (*Print[sign[[i]]];*)
