...
wilsonLoopold[h_,wccold_,occ_,start_,end_]:=Module[
{rnk1,path,evals,evecs,eigvec,hk,hkdag,eig0,evec0,
V,P,W,Wij
},
rnk1=50;
V=DiagonalMatrix[Table[Exp[-I #1 . tau],{tau,wccold}]]&;
...
plotWilsonLoopold[h_,wccold_,occ_,start_,end_,pathstr_,npoint_]:=Module[{hkp,hkps,xticks,data},
...
data=Transpose@Table[wilsonLoopold[h,wccold,occ,start+i,end+i],
{i,#}]&/@(Partition[tokpathvaspold[pathstr,#],#+1]&@npoint);
...
z2pathold[h_,occ_,path1_]:=Module[
...
berryphold[h_,occ_,path1_]:=Module[
...
wLoopold[h_,occ_,path1_,path2_]:=Module[
