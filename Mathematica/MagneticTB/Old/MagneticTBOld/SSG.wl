...
findnSSG[cold_, lbond_, lattplot_] := 
  Module[{atom, natom, nwyck, mksc, sizesc, sc, split}, 
   ...
   Table[
    split@Sort[
      Table[{EuclideanDistance[cold[[wkp, wkpeqvi]] . lattplot, 
     ...
     Length[cold[[wkp]]]}, {scwkp, nwyck}, {scwkpeqvi, 
     Length[cold[[scwkp]]]}]];


initSSGold[inputDict_] := Module[
   {inputAssociation, x1, y1, z1,
    spinMatrix, spinMatrix2, directSum, timesSSG, identityElement,
    latticeold, lattparold, wyckoffpositionold, symminformationold, basisFunctionsold,
    lattplot, reclattold, reclattplot, volume, volumeplot, symmop, 
    Allsymmop, Gensymmop,
    atominfo, atomposold, pos, magdirection, spinrot, rotorb, rotspin, 
    rule,
    ddim, repmats,
    base, pbase, nbases,
    coeff, opmatrix, sol,
    symopwyck, symmetryopsold,
    bondclassifyold, wccold
    },
   
   spinMatrix[op_] := 
    FullSimplify[
     Module[{alphaold, betaold, gammaold, nop}, 
      If[Det[op] == -1, nop = -op, nop = op];
      {alphaold, betaold, gammaold} = -EulerAngles[nop];
      Transpose@{{E^(I gammaold/2) Cos[betaold/2] E^(I alphaold/2), 
         E^(I gammaold/2) Sin[betaold/
            2] E^(-I alphaold/2)}, {-E^(-I gammaold/2) Sin[betaold/
            2] E^(I alphaold/2), 
         E^(-I gammaold/2) Cos[betaold/2] E^(-I alphaold/2)}}]];
   ...
   latticeold = 
    inputAssociation[
      "lattice"] /. {xold_?InexactNumberQ :> 
       Rationalize@Round[xold, .001]};
   lattparold = 
    inputAssociation[
      "lattpar"] /. {xold_?InexactNumberQ :> 
       Rationalize@Round[xold, .001]};
   wyckoffpositionold = 
    inputAssociation[
       "wyckoffposition"] /. {xold_?InexactNumberQ :> 
        Rationalize@Round[xold, .001]} /. {"x" -> xold, "y" -> yold, 
      "z" -> zold};
   
   symminformationold = 
    inputAssociation[
      "symminformation"] /. {xold_?InexactNumberQ :> 
       Rationalize@Round[xold, .001]};
   
   
   
   lattplot = latticeold /. lattparold;
   volume = Cross[latticeold[[1]], latticeold[[2]]] . latticeold[[3]];
   reclattold = 2  Pi/volume  {
      Cross[latticeold[[2]], latticeold[[3]]], 
      Cross[latticeold[[3]], latticeold[[1]]],
      Cross[latticeold[[1]], latticeold[[2]]]
      };
   reclattplot = reclattold /. lattparold;
   volumeplot = volume /. lattparold;
   
   symmop = Values[symminformationold];
   Allsymmop = GenerateGroupold[symmop, identityElement, timesSSG];
   
   Gensymmop = getGeneratorold[Allsymmop, identityElement, timesSSG];
   ...
   atomposold = Table[
     Table[
      ...
      {pos, magdirection}
      , {sym, Allsymmop}],
     {wyck, wyckoffpositionold}];
   atomposold = DeleteDuplicates /@ atomposold;
   
   
   
   If[Max[Length /@ Values[GroupBy[Flatten[atomposold, 1], #[[1]] &]]] > 
     1,
    Print[
...
more than one atoms.\n", atomposold]; Abort[]];
   ...
   Do[
    ...
    Do[
     ...
     Which[
      ...
      sol = First@SolveAlways[pbase == opmatrix . base, {xold, yold, zold}];
      ...
      rule = 
       FullSimplify@
        MapThread[
         Rule, {{xold, yold, zold}, 
          FullSimplify@(Inverse[
              sym["space"][[1]]] . ({xold, yold, zold} . Inverse[lattplot]) . 
             lattplot)}];
      If[sym["spin"][[2]] == 1,
       pbase = 
      ...
      sol = 
       Flatten[Table[
         SolveAlways[(pbase[[j]]) == 
           Total@Table[opmatrix[[j, k]] base[[k]], {k, nbases}], {xold, 
           yold, zold}], {j, nbases}]];
   ...
   symmetryopsold = 
    Association[
     Thread[Allsymmop -> 
       Table[directSum @@ i, {i, Transpose[symopwyck /@ atomposold]}]]];
   bondclassifyold = 
    Split[SortBy[
      Flatten[findnSSG[atomposold[[;; , ;; , 1]], 1, lattplot], 
       4], #[[1]] &], #1[[1]] == #2[[1]] &];
   bondclassifyold = 
   ...
   wccold = 
    Module[{natomperwyck, nbasesperwyck}, 
     ...
     nbasesperwyck = Length /@ wyckoffpositionold;
     Flatten[
      Table[Table[
        Table[atomposold[[i, j, 1]], nbasesperwyck[[i]]], {j, 
         natomperwyck[[i]]}], {i, Length[natomperwyck]}], 2]];
   
   <|"atompos" -> atomposold, "symmetryops" -> symmetryopsold, "wcc" -> wccold,
     "bondclassify" -> bondclassifyold, "input" -> inputAssociation, 
    "lattplot" -> lattplot, "reclatt" -> reclattold, 
...
unsymhamSSG[n_, bondclassifyold_, atomposold_, pointops_] := Module[
   {bondends, bandind, hopold, bonds, toband, nbh, hami, tobond},
   tobond = #[[2]] - #[[1]] &;
   ...
   Do[h[toband[bandind[[i, 1]]], toband[bandind[[i, 2]]]] += 
      Exp[I ({kxold, kyold, kzold}) . (bonds[[i]])] hopold[bandind[[i]]];, {i, 
     Length[bondends]}];
   hami = 
    ArrayFlatten[Table[h[i, j], {i, Total@nbh}, {j, Total@nbh}]]];


Options[symhamSSGold]={"symmetryset"->All};
(*SetOptions[symham,symmetryset\[Rule]All];*)
symhamSSGold[n_,symmetry_,OptionsPattern[]]:=Module[
{conjh,h,phpmh,recR,para,sset,opset,symmetryinfo,unham,
opsold},
opsold=KeyTake[symmetry["symmetryops"],symmetry["Gens"]];
sset=OptionValue["symmetryset"];
If[sset===All,opset=Range[Length[opsold]],opset=sset,opset=Range[Length[opsold]]];
symmetryinfo=Normal[opsold];
unham=unsymhamSSG[n,symmetry["bondclassify"],symmetry["atompos"],symmetry["pointops"]];
conjh=(ComplexExpand@ConjugateTranspose@unham)-unham;If[n==1,h=unham/. ToRules@Reduce[conjh==0],h=unham/. ToRules@Reduce[Flatten[Map[Total/@(Values[GroupBy[Cases[#,xold_ E^yold_:>{xold,Expand[yold]}],Last]][[;;,;;,1]])&,TrigToExp[conjh],{2}]]==0];h=h/. ToRules@Reduce[Flatten[Map[Total/@(Values[GroupBy[Cases[{#},xold_ E^yold_:>{xold,Expand[yold]}],Last]][[;;,;;,1]])&,TrigToExp[conjh],{2}]]==0]];phpmh=Table[
recR=Transpose[symmetryinfo[[i,1]]["space"][[1]]];Which[symmetryinfo[[i,1]]["spin"][[2]]==0,(Inverse[symmetryinfo[[i,2]]] . h . symmetryinfo[[i,2]])-(h/. Thread[{kxold,kyold,kzold}->recR . {kxold,kyold,kzold}]),symmetryinfo[[i,1]]["spin"][[2]]==1,(Expand@TrigToExp@(Inverse[symmetryinfo[[i,2]]] . (h) . symmetryinfo[[i,2]]))-Expand@TrigToExp@(ComplexExpand@Conjugate[h/. Thread[{kxold,kyold,kzold}->recR . {-kxold,-kyold,-kzold}]])],{i,opset}];

phpmh=Expand[phpmh];If[n==1,(*Print[First@Solve[phpmh\[Equal]0]];*)h=h/. ToRules@Reduce[phpmh==0],phpmh=Join[Map[Total@Cases[#,xx_ E^yy_:>xx E^Simplify@yy]&,phpmh,{3}],Map[Total@Cases[{#},xx_ E^yy_:>xx E^Simplify@yy]&,phpmh,{3}]];h=h/. FullSimplify@ToRules@Reduce[DeleteDuplicates@Flatten[Map[Total/@(Values[GroupBy[Cases[#,xold_ E^yold_:>{xold,Expand[yold]}],Last]][[;;,;;,1]])&,phpmh,{3}]]==0];h=h/. FullSimplify@ToRules@Reduce[DeleteDuplicates@Flatten[Map[Total/@(Values[GroupBy[Cases[{#},xold_ E^yold_:>{xold,Expand[yold]}],Last]][[;;,;;,1]])&,phpmh,{3}]]==0];];para=Thread[Which[n==1,#->Table[ToExpression["e"<>ToString[i]<>"old"],{i,Length[#]}]&@Variables[h],n==2,#->Table[ToExpression["t"<>ToString[i]<>"old"],{i,Length[#]}]&@Variables[h],n==3,#->Table[ToExpression["r"<>ToString[i]<>"old"],{i,Length[#]}]&@Variables[h],n==4,#->Table[ToExpression["s"<>ToString[i]<>"old"],{i,Length[#]}]&@Variables[h],True,#->Table[ToExpression["p"<>ToString[n]<>"n"<>ToString[i]<>"old"],{i,Length[#]}]&@Variables[h]]];h=h/. para;Print["params:",Variables[h]];
h];
