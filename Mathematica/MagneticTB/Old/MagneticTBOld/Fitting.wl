...
bandManipulateEigold[h_, eigdata_] := Module[
     ...
     h0 = TrigToExp[h];
     params = Variables[h0];
     mparams = ToExpression["p" <> ToString[#] <> "old" & /@ params];
...
tokpathvaspold[pathstr_,npoint_]:=2Pi Flatten[Subdivide[#[[1]],#[[2]],npoint]&/@ToExpression@Partition[Partition[StringSplit[pathstr],5][[;;,1;;3]],2],1];
...
fittingTB[h_, eigdata_, krange_, initparms_] := Module[
  ...
  params = Complement[Variables[h], {kxold, kyold, kzold}];
  klist = eigdata[[;; , 1]];
  nk = Length[krange];
  mparams = params;
  hp = h /. Thread[params -> mparams];
  modelhams = 
   Table[hp /. {kxold -> k[[1]], kyold -> k[[2]], kzold -> k[[3]]}, {k, 
     2 Pi klist[[krange]]}];
  res = FindMinimum[
    Total@Table[
...
vaspEigold[filename_,efermi_,spin_,startband_,endband_]:=Module[
...
Options[compareBandold]= {plotRangeold->All};
compareBandold[pathstr_,npoint_,ham_,rule_,vaspband_,OptionsPattern[]]:=
  ...
  Transpose@Table[Sort@Eigenvalues[Evaluate[ham/.rule/.{kxold->kps[[k,1]],kyold->kps[[k,2]],kzold->kps[[k,3]]}]],{k,Length[kps]}]
  ,{i,Length[tmp]}];
